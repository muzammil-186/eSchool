...
@WebServlet("/TuitionDetailServlet")
public class TuitionDetailServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			List<TuitionFee> list = new ArrayList<TuitionFee>();
			list = tuitionFeeDao.getAllTuitionFee();
			request.setAttribute("fees", list);
		//	System.out.println("EnrhServlet-doGet we r here!!");
		//	String pathInfo = request.getPathInfo();
			request.getRequestDispatcher("/WEB-INF/views/login/TuitionFee.jsp").forward(request, response);
